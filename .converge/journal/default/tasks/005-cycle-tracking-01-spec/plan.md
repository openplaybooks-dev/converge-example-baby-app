# Plan: Spec: Cycle Tracking



Based on my exploration, I have a complete picture of the codebase. Let me synthesize the Cycle Tracking specification.
...
| `titleLarge` | 20sp | 600 | 28pt | AppBar title |
| `titleMedium` | 16sp | 600 | 24pt | Card titles, list item titles |
| `bodyLarge` | 16sp | 400 | 24pt | Primary body text |
| `bodyMedium` | 14sp | 400 | 20pt | Secondary text, descriptions |
| `labelLarge` | 14sp | 600 | 20pt | Button text, chip labels |
| `caption` | 12sp | 500 | 16pt | Badges, timestamps |
...
1. **`.stitch/UX.md`** — Contains the screen specification, scaffold requirements, and data entity definitions for `CycleEntry` and `CycleEntryProvider`

2. **`.stitch/system/DESIGN.md`** — Contains design tokens (colors, typography, spacing, motion) that must be referenced for all visual styling

3. **`lib/screens/browse/browse_screen.dart`** — Existing screen with `_LinkCard` pattern showing navigation to `/cycle-tracking` — reference for card tap animations and navigation patterns

4. **`.stitch/designs/home/SPEC.md`** — Completed spec following the same format structure; reference for section documentation patterns, data entity tables, and accessibility specifications

5. **`lib/router/app_router.dart`** — Router configuration where `/cycle-tracking` route must be registered; explore to understand current route structure and how to add the new screen