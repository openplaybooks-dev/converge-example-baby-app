# Plan: Spec: Mindfulness



Now I have all the context needed. Let me create the comprehensive specification plan for the Mindfulness screen.

---

## Spec: Mindfulness — Design Plan
...
**Colors (Light Theme)** — from `DESIGN.md`:
| Token | Hex | Usage |
|---|---|---|
| `lavenderBloom` | `#7C3AED` | Primary brand, AppBar, active states |
| `blushRose` | `#F9A8D4` | Secondary accent, mood elements |
...
**AppBar Behavior:**
- Background: `lavenderTint` (#F5F3FF), transparent effect
...
**Static Exercise/Routine Catalog:**
- `BreathingExercise`: id, name, durationSeconds, difficulty, description
- `StretchingRoutine`: id, name, durationMinutes, trimesterSuitability, steps (List of step descriptions)
...
**No Provider Dependency for MVP:**
- Exercise and routine data is static (hardcoded in the screen or a static catalog class)
...
4. **`.stitch/designs/mindfulness/SPEC.md`** — Target output file (this specification document)
