---
id: 002-browse-04-analyze
title: "Analyze: Browse"
description: Identify extractable widget regions in Browse
inputs:
  - lib/screens/browse/browse_screen.dart
outputs:
  - .stitch/designs/browse/widgets.jsonl
checks:
  - id: widgets-json-exists
    type: cmd
    cmd: test -f .stitch/designs/browse/widgets.jsonl
    description: widgets.jsonl exists for browse
plan: {}
tags:
  - analyze
  - screen-browse
vars:
  ...
  widgetsJsonPath: .stitch/designs/browse/widgets.jsonl
---
...
## Input
- `lib/screens/browse/browse_screen.dart` — The screen widget file

## Task

Read the screen file and identify widget subtrees that should be extracted into separate widget files.

Write `.stitch/designs/browse/widgets.jsonl` with one JSON object per line (JSONL format):
...
- `.stitch/designs/browse/widgets.jsonl` exists with valid JSONL
