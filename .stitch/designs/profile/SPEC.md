...
| Token | Hex | Usage |
|---|---|---|
| `twilightBloom` | `#9F67FF` | Primary brand, active nav indicator |
| `duskyRose` | `#FBC4E0` | Secondary accent, Mindfulness icon |
| `nightMint` | `#34D399` | Tertiary, success states |
| `deepTwilight` | `#1E1B2E` | Card backgrounds |
| `nightSky` | `#13111C` | Scaffold background |
| `softPearl` | `#F3F4F6` | Primary text |
| `mutedSilver` | `#9CA3AF` | Secondary text, descriptions |
| `deepBorder` | `#2D2B3A` | Card borders |
| `charcoalSurface` | `#252336` | Bottom nav background, card surfaces |
...
| `caption` | 12sp | 400 | 16sp | Badges, timestamps |
...
## 12. Navigation Dependencies

| Destination | Route | Navigation Type |
|---|---|---|
...
## 13. Theme Dependencies

This screen uses `Theme.of(context)` to access design tokens. The theme is configured via `app_theme.dart` which provides:
- `ColorScheme` with all palette colors mapped
- `TextTheme` with Nunito font family
- `CardTheme` with proper corner radius and elevation